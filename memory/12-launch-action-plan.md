...
- [x] **동시성 버그 2건 수정 + Android 실기기 재검증(2026-07-14, 코덱스 findings 1·2·4)**:
  - **client.ts refresh single-flight**(finding1, HIGH): 동시 401들이 refresh 중복발사 → 백엔드 원자적 회전(내가 전날 하드닝)에 걸려 정상유저 오로그아웃 되던 회귀. single-flight로 한 번만 발사·공유. 동시성 유닛테스트 추가(27/27). 커밋 e292b65
  - **TimerScreen 재진입 가드**(finding2): handleStartStop에 startStopBusy ref — 더블탭 세션중복 방지
  - **finding3(릴리즈 서명)**: 소스버그 아님. `/android` gitignore(prebuild 산출물, `git ls-files`=0), build.gradle의 debug keystore는 로컬 dev용. EAS production은 독립 credentials로 서명 → 첫 프로덕션 빌드 시 `eas credentials` 확인만
  - **finding4(HEAD 미검증) → 해소**: SM-A165N(Android16) dev-client에 Metro로 현재 HEAD JS 로드(★ Docker가 8081 점유 → Metro 8085, 번들URL이 8085 가리켜 `adb reverse tcp:8081→8085`만으론 실패 → `tcp:8085 tcp:8085`도 매핑해야 로드됨). 검증: 로그인세션 유지(하드닝백엔드+CF strict 정상), SDUI 배너 렌더, 타이머 시작/종료 풀사이클, **더블탭→세션1개만(오늘총 48초=단일세션분, 고아 없음=가드작동)**, 업무기록모달+할일로드, 기록(달력/7월1분), 할일탭 전부 정상. 크래시0·API실패0. 증거 `/tmp/filltime-qa/*.png`
- [x] **★ edge-to-edge 시스템바 겹침 근본 수정(2026-07-14, 커밋 8128a2a)** — 사용자 보고: 하단 컨텐츠가 안드 뒤로가기/홈 내비바와 겹침. 원인: `edgeToEdgeEnabled=true`(시스템바 뒤로 렌더)인데 `react-native-safe-area-context`를 앱 어디서도 미사용(SafeAreaProvider/insets 0건) → 하단이 내비바 밑으로. 특히 세션종료 모달 건너뛰기/저장 버튼이 내비바에 붙어 탭 불가(실제로 QA 때 adb탭 안 먹었음).
  - 수정: App.tsx 루트 `SafeAreaProvider`(→react-navigation 탭바 하단·헤더 상단 자동 인셋), SessionEndModal·ShareCardModal 바텀시트 `paddingBottom=insets.bottom+16`, OnboardingScreen 하드코딩 StatusBar.currentHeight→`useSafeAreaInsets`, LoginScreen 상/하단 인셋
  - 실기기 before/after 검증: 탭바·모달버튼이 내비바 위로 여백 확보(`/tmp/filltime-qa/A0*.png` vs 이전 11·13). onboarding/login은 로그아웃 필요해 온디바이스 재검증 생략(동일 인셋 패턴+tsc통과)
  - **교훈**: Expo SDK54+ Android는 edge-to-edge 기본 → 신규 RN앱은 처음부터 SafeAreaProvider + 전화면 인셋 필수. 앱 팩토리 템플릿에 기본 반영할 것
