...
- 진행(2026-07-24): ✅ **iOS 3차 제출 완료** — 1.0.1(build6)+구독2 = WAITING_FOR_REVIEW(03:52). 1·2차 반려(3.1.2 EULA링크→3.1.2c 페이월링크+3.1.1 복원버튼) 전부 해소 + Codex감사(방침 Anthropic위탁·ASC프라이버시라벨 7종). **Android vc4 = 정상 심사중**(12테스터 아님, "전송안됨"은 vc1 draft 표시노이즈). 게이트: iOS 07-25~26 / Android 07-27. **심사중 콘솔 노터치.** 상세 32번 최상단
- 진행(2026-07-22): ⚠️ iOS v1.0.1 REJECTED + Android vc4 미게시 의심 → **둘 다 07-24에 해소/정상확인됨(위 참조)**
...
| `31-session-2026-07-19.md` | ★ 세션종합(07-19) — **압축 후 재개 1순위**. 자정버그·목금데이터이관·테마·대시보드·연속기록KST·AI스트리밍(코덱스) + 배포현황·다음작업·커밋맵 |
