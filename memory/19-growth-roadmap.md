...
> **07-19 정정**: ①아래 "Android RC offering 완료(라인58)"는 **현재 틀림** — Play 상품은 ACTIVE인데 **Android RC `default` 오퍼링 패키지 0개**(SDK 실측). iOS만 실동작. Android는 RC 대시보드에서 오퍼링↔Android상품 연결 + 내부테스트 검증 필요(P1). ②AI분석·위젯·푸시는 **구현 완료**(아래 "현재 자산"의 미구현/locked 표기는 stale).
