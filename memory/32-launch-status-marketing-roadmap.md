...
- **Android vc4 = production 트랙 completed인데 미게시**(공개 404, vc1 draft 찌꺼기도 있음). 유력 원인 2개:
  ...
  ⚠️ 재제출·미게시 전환 금지(심사 시계 리셋됨). 07-20 03:27 제출이라 07-27 전엔 에스컬레이션도 이르다.
- **다음 액션**: ①ASC Resolution Center에서 iOS 반려 사유 확인(브라우저 필요 — playwright는 타 세션이 잠금, computer-use나 직접) ②Play Console 대시보드에서 테스트 요건 배너 확인 ③사유별 대응.
- 만약 ①이 12테스터 요건이면: 내부테스트 트랙에 vc4 올리고 테스터 12명+ 모아 14일 러닝 → 그동안 iOS 반려 수정에 집중.
