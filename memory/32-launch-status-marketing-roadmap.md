...
- **vc5 빌드·제출 완료(07-28)**: versionCode 4→5, production 트랙 `status=completed`(관리형 게시 꺼짐 = 승인 즉시 자동 배포).
  vc4에 없던 것 5종 탑재 — 구독 관리·해지 버튼(Play 정책 요건), 페이월 갱신 금액 고지,
  결제 퍼널 계측(취소/실패 구분), **AI 동의 화면**(백엔드 게이트가 이미 라이브라 가장 급했음),
  타이머 AppState 재동기·알림 64한도/경합방지·OTA 즉시재시작 제거.
- **구독 해지**: Play에서 `취소됨(2026-08-03 종료)` 확정 → ₩29,000 미청구.
  ⚠️ 단 **RC `CANCELLATION` 웹훅이 30분 넘게 미수신** — 서버는 여전히 `trial|cancelled=f`.
  동작상 영향은 없다(`isPremium`이 cancelled도 만료 전까진 프리미엄 인정). 다만 **RC 웹훅
  이벤트 커버리지 점검 필요** — 해지/만료가 서버에 안 들어오면 만료 후에도 프리미엄이
  남는 버그로 이어질 수 있다. 8/3 만료 시 EXPIRATION 수신 여부로 재확인할 것.
- 남은 것: 복원(restore) E2E — vc5 승인 후 폰에서 재설치→복원→서버 프리미엄 유지 확인(구독 8/3까지 유효).
